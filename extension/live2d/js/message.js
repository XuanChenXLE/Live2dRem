...
(function () {
  ...
  function initLive2d() {
    ...
    // 扩展 iframe 只有 300px 宽，恒被判成手机，所以扩展内默认非全屏（手动切全宽仍会被记住）
    function isMobileWidth() {
      return window.matchMedia && window.matchMedia('(max-width: 860px)').matches;
    }
    var fullPref = null;
    try { fullPref = localStorage.getItem('remFull'); } catch (e) {}
    if (fullPref === '1' || (fullPref === null && !window.__REM_EXTENSION__ && isMobileWidth())) {
    ...
    // 拖动：鼠标 + 触屏
    // 扩展 iframe 内（window.__REM_EXTENSION__，见 extension/rem.html）：
    // landlord 固定在 iframe 里不动，拖动改为上报位移，由父页面移动 iframe 本体，
    // 这样模型可全屏拖动；同时上报内容区尺寸，父页面收紧/放大 iframe，气泡不再被裁。
    var smcc = document.getElementById('landlord');
    if (window.__REM_EXTENSION__) {
      setupExtensionBridge(smcc);
    } else {
  ...
  // ---- 扩展桥：只在 extension/rem.html 里启用（__REM_EXTENSION__） ----
  function setupExtensionBridge(smcc) {
    function post(d) { try { parent.postMessage(d, '*'); } catch (e) {} }
    function isUiTarget(t) {
      if (!t || !t.closest) return false;
      return !!t.closest('input,button,textarea,select,.live_talk_input_body,.live_ico_box,#remResize');
    }
    // 上报 iframe 需要的尺寸：landlord 与气泡 .message 的并集（气泡 absolute 不撑开父盒，必须手动并）
    function reportFrame() {
      try {
        var full = smcc.classList.contains('rem-full');
        var lr = smcc.getBoundingClientRect();
        var top = lr.top, left = lr.left, right = lr.right, bottom = lr.bottom;
        if (!lr.width) { // landlord 隐藏时（召唤蕾姆按钮状态）按按钮算
          var ob = document.getElementById('open_live2d').getBoundingClientRect();
          top = ob.top; left = ob.left; right = ob.right; bottom = ob.bottom;
        } else {
          var msg = smcc.querySelector('.message');
          if (msg) {
            var mr = msg.getBoundingClientRect();
            if (mr.width > 0 && mr.height > 0) {
              if (mr.top < top) top = mr.top;
              if (mr.left < left) left = mr.left;
              if (mr.right > right) right = mr.right;
            }
          }
        }
        post({ __rem: true, type: 'frame', full: full,
          width: Math.ceil(right + 10), height: Math.ceil(bottom - top + 10) });
      } catch (e) {}
    }
    // 指针捕获：按下后后续 move/up 全重定向到 landlord，
    // 鼠标拖出 iframe 框也不会断流，这是顺滑的关键（Chromium 134 支持 Pointer Events）
    var sx = 0, sy = 0, dragging = false;
    smcc.addEventListener('pointerdown', function (e) {
      if (isUiTarget(e.target)) return;
      dragging = true; sx = e.clientX; sy = e.clientY;
      try { smcc.setPointerCapture(e.pointerId); } catch (err) {}
      post({ __rem: true, type: 'dragstart' }); // 父页面据此在拖动中忽略尺寸上报，互不打架
      e.preventDefault();
    });
    smcc.addEventListener('pointermove', function (e) {
      if (!dragging) return;
      post({ __rem: true, type: 'drag', dx: e.clientX - sx, dy: e.clientY - sy });
      sx = e.clientX; sy = e.clientY;
    });
    function endPointerDrag() {
      if (!dragging) return;
      dragging = false;
      post({ __rem: true, type: 'dragend' });
    }
    smcc.addEventListener('pointerup', endPointerDrag);
    smcc.addEventListener('pointercancel', endPointerDrag);
    if (window.ResizeObserver) { try { new ResizeObserver(reportFrame).observe(smcc); } catch (e) {} }
    if (window.MutationObserver) {
      try { new MutationObserver(reportFrame).observe(smcc, { attributes: true, attributeFilter: ['class', 'style'] }); } catch (e) {}
    }
    setInterval(reportFrame, 1500); // 气泡显隐/缩放的兜底
    setTimeout(reportFrame, 800);
    var fb = document.getElementById('fullButton');
    if (fb) fb.addEventListener('click', function () { setTimeout(reportFrame, 80); });
  }
